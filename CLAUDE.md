...
| 区分 | 置き場所 |
|---|---|
| 自分のポートフォリオ | `data/output/portfolio/myself.tsv` |
| テンバガーXのポートフォリオ | `data/output/portfolio/tenbagger_x.tsv` |
| お気に入り | Googleスプレッドシート（まだTSVに書き出していない。`docs/TODO.md` 参照） |

コードから対象を取るときは `collectors/holding_profile_dump.py` の `portfolio_codes()`
を使う。`data/output/portfolio/*.tsv` を読むので、お気に入りのTSVを足せば自動で対象に入る。
...
- **拠点数はタグ付けされていない**。本文から拾うしかなく、「導入先の店舗数」
  「改装した店舗数」「翌期の計画値」「市場規模」を自社の拠点として拾う事故が起きる。
  保有銘柄は読んで確かめ、誤りは `facility_override.tsv` に書く
