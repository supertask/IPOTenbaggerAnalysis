...
#
# 3つめは、その型が投資家にとって何を意味するかの一言。型ごとに決まる話なので
# AIに書かせる必要はない。会社ごとの事情（なぜ売出しをしたのか）は書類の
# 本文にしかないので、そちらは適時開示のPDFへのリンクで辿ってもらう
_REASON_LABELS = (
    ...
    ("第三者割当", ("第三者割当",),
     "特定の相手に新株を割り当てた。資本提携か資金調達"),
    ("自己株買い", ("自己株式",),
     "会社が自社株を買った。1株あたりの価値は上がる"),
...
# 開くたびに下の行がずれて読みにくい
_LOCKUP_END = re.compile(r"から\s*(\d{4})年\s*(\d{1,2})月\s*(\d{1,2})日\s*までの期間")
_PLEDGE = re.compile(r"([\d,]+)\s*株を?(?:担保|質権)")
_LENT = re.compile(r"([\d,]+)\s*株を?(?:貸付|貸借|借入)")
...
def reason_label(text: str) -> str:
    ...
    for label, words, _ in _REASON_LABELS:
        if any(w in text for w in words):
            return label
    return ""
...
def get_large_holdings(company_code) -> Optional[dict]:
    ...
    for row in sorted(rows, key=lambda r: (r.get("提出日") or "", r.get("保有者") or "")):
        ...
        events.append({
            ...
            "why": label,
            "why_note": _REASON_NOTES.get(label, ""),
    ...
    for e in events:
        ...
        if e["transfer"] and not e["why"]:
            e["why"] = "関係者間で移動"
            e["why_note"] = _REASON_NOTES["関係者間で移動"]
