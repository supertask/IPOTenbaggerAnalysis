...
# 1銘柄1行。事業・経営陣・財務指標の読み方・総括を1つのTSVで持つ。
# 以前は財務指標だけ metric_reading.tsv に分けていたが、同じ材料（有報・業績・
# 競合）を2回読むことになるうえ、株価水準の欄で内容が重複していた
...
# 財務指標の比較カードに出す
METRIC_SECTIONS = [
    ("見どころ", "30枚のグラフのうち、まずどれを見るか"),
    ("競合との差", "同じ土俵の会社・10倍になった会社と並べて何が違うか"),
    ("数字の注意", "そのまま読むと誤るところ"),
]
...
def get_metric_reading(company_code) -> Optional[dict]:
    ...
    比較チャートは30枚近くあり、どれから見ればいいかが分からない。
