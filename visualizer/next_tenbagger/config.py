...
# %で見る指標。データは 0.24 のような小数で入っているので、
# グラフに出すときに100倍して「24%」にする。
# 以前は営業利益率とROEだけがチャート側にべた書きされていて、
# ROA・自己資本比率・配当性向などが 0.15 のまま出ていた。
#
# 倍率で読むもの（PER・PEG・PSR・ネットキャッシュ比率・有利子負債÷純資産・
# 利益の質・キャッシュニュートラルPER）はここに入れない。0.05倍が正しい表示。
PERCENT_METRICS = frozenset({
    'ROE（自己資本利益率）', 'ROA（総資産利益率）', '自己資本比率',
    '営業利益率', '売上総利益率', '配当性向',
    '潜在株式による希薄化率', '臨時雇用の比率', '在庫の伸び − 売上の伸び',
})

# このうち **XBRLから直に来る** もの。会社によって 0.34 で入っていたり
# 34 で入っていたりするので、系列の大きさを見て100倍するか決める。
# それ以外（営業利益率・配当性向など）はこちらで小数として計算しているので、
# 常に100倍でよい。配当性向は減益期に200%を超えることがあり、
# 「大きいから既に%だろう」と判定すると外れる
PERCENT_METRICS_FROM_XBRL = frozenset({
    'ROE（自己資本利益率）', '自己資本比率',
})
...
CHART_DISPLAY_ORDER = [
    ...
    'キャッシュニュートラルPER',    # 清原。現金の多寡を揃えた実質のPER
